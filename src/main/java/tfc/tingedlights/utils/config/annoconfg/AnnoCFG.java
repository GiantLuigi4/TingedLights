...
import tfc.tingedlights.utils.config.annoconfg.annotation.format.*;
import tfc.tingedlights.utils.config.annoconfg.annotation.value.*;
import tfc.tingedlights.utils.config.annoconfg.handle.UnsafeHandle;
import tfc.tingedlights.utils.config.annoconfg.util.EnumType;
			...
			if (field.canAccess(null)) {
				...
				switch (EnumType.forClass(field.getType())) {
					...
					case FLOAT -> {
						...
						float v = defaultValue.valueF();
						if (range != null) {
							float min = range.minV();
							float max = range.maxV();
							
							Supplier<Double> dvalue = builder.defineInRange(nameStr, v, min, max);
							value = () -> dvalue.get().floatValue();
						} else {
							value = builder.define(nameStr, v);
						}
					}
